...
//     const whatsappUrl = `https://wa.me/+919705947947?text=${message}`;
...
const fallingLeaves = Array.from({ length: 12 }).map((_, i) => ({
  id: i,
  ...
  return (
    <div className="relative min-h-screen bg-green-50 overflow-hidden">
      ...
      {fallingLeaves.map((leaf) => (
        <motion.div
          key={leaf.id}
          className="absolute text-green-300 opacity-50"
        ...
        >
          <Mail style={{ width: leaf.size, height: leaf.size }} />
        </motion.div>
      ))}
